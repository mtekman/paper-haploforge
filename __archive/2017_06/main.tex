...
%\usepackage{assets/biotemplate/float}
\usepackage{amsmath}
...
% App defns
\def\haplo{{HAPLOSCOPER}}            % inTREPID 	  haploType REsolution Pedigree Invention Device
\def\hpainter{{HaploPainter}}


\newcommand{\figeps}[4][1]{\
\begin{figure}[!tpb]
	\centerline{\includegraphics[scale=#1]{#3}}\caption{#4}\label{#2}
\end{figure}
}
...
\textit{Web Application and Source Code:}\ \ \url|https://www.bitbucket.io/mtekman/haplo_html5|\\\
\textbf{Supplementary information:} Supplementary data is available from \textit{Bioinformatics} online.\\
\textbf{Contact:} \href{r.kleta@ucl.ac.uk}{r.kleta@ucl.ac.uk}\  % headaches
}

\maketitle

\section{Introduction}
%Family studies remain an important approach for investigating the aetiology of monogenic disorders.
...
%
%%% > speculative, it would be perfect if it was a flaw in their method and not just a bug, needs to be reworded if just a bug
%
%%% From what I've been able to understand going through their code, they have no contingency for handling X-linked and I semi-confirmed this with an email to the developer a while back.
%
...
\fig{fig:hpainterx}{imgs/hpainter_inheritance_small.png}
...
%
%In contrast to \hpainter{}, where regions of interest need to be found manually in each successive generation, 

%%% > is this true of any other program?
%%% Not as far as I've seen, working on it. See other_apps.txt

\fig{fig:dos}{imgs/dos2.png}
...
\fig{fig:viewportmanip}{imgs/all_in_one.png}
...
\fig{fig:pathfind}{imgs/path_finder.png}
...
\fig{fig:graphpath}{imgs/graph_and_path2.png}
...
the graph initialization procedure does not make %any assumptions on which allele is maternal or paternal
...
%%% > Also, does this make sense? Two paths can surely have the same number of recombinations but be very different, right?
%%% In theory yes, but in practice the size of each haploblock in a given marker stretch varies a lot for blocks larger than ~3 markers, so you tend to end up with a pretty mixed number of total recombinations over the same stretch. 
%%% When you have multiple paths sharing the same number of recombinations, it's usually just an extension of the exact same path with very little branching previous to it. This probably means that said path *is* optimal, but just in case I keep a few extra running to create more 'diversity'. 

\input{resources/floats/pseudo}
...
\fig[0.8]{fig:homology}{imgs/homology.png}
...
\fig{fig:xcomp}{imgs/x_compare.jpg}
...
\input{resources/jabber}
\input{resources/biblio/bibliography}
